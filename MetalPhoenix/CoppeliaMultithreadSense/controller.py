...
from utility import short_way, detect_target, StdoutLogger
...
class Controller:
    def __init__(self):
        ...
        if not self.__align_robot(target=90):
            self.__class_logger.log("Alignment fail!", 3)

        self.body.thread_kill(ThreadType.th_orientation)

    def __align_robot(self, target: None | float = None, timeout: float = 3.0) -> bool:
        """Align robot to target.
        #WARNING: To work it needs the 'orientation' thread to be alive.

        #PARAMS -> target: None | float. Target to align robot,
                           if it's None, the '__detect_target()' method will be invoked
                           to calculate the nearest correct alignment.
                -> timeout: float. Do action until timeout is not reached.

        #RETURN: bool. True if alignment is reached else False.

        Blocking method until the target is reached or timeout is reached.
        """
        ...
        while ori > (target + 1) or ori < (target - 1):
            ori = self.body.get_orientation_deg()
            if not executed:
