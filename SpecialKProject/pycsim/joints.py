...
class AnyJoint:
    ...
    def set_target_velocity(self, target):
        s.simxSetJointTargetVelocity(
            self._id, self._handle, target, self._def_op_mode)

    def set_target_position(self, target):
        s.simxSetJointTargetPosition(
            self._id, self._handle, target, self._def_op_mode)

    def get_force(self):
        code, force = s.simxGetJointForce(
    ...
    def get_position(self):
        code, position = s.simxGetJointPosition(
            self._id, self._handle, self._def_op_mode)
        return position

    def get_matrix(self):
        code, matrix = s.simxGetJointMatrix(
            self._id, self._handle, self._def_op_mode)
        return matrix

    def set_matrix(self, matrix):
        assert len(matrix) == 12
        s.simxSetSphericalJointMatrix(
...
class Joints:
    ...
    def spherical(self, name: str) -> SphericalJoint:
        ...
        joint = self._get_joint_with_param(
            name,
            [sc.sim_joint_spherical_subtype],
            sc.sim_jointmode_passive)
    ...
    def spring(self, name: str) -> SpringJoint:
        ...
        """
        joint = self._get_joint_with_param(
            name, [sc.sim_joint_revolute_subtype, sc.sim_joint_prismatic_subtype],
            sc.sim_jointmode_force)
    ...
    def _get_joint_with_param(self, name, types, mode) -> AnyJoint:
        ...
        if handle is not None:
            typ, curr_mode, limit, rng = self._get_info_about_joint(handle)
            if typ in types and curr_mode == mode:
